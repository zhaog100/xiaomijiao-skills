...
## 🎯 当前状态（2026-03-21 15:05）

**FinMind PR：** 22个→rohitdash08/FinMind，0 merged 0 review（已follow up）
**APort PR：** 15个→aporthq/aport-integrations，14 open，0 review（已follow up）
**估算总价值：** $4,350-4,850（全部等待review）
**ClawHub总发布：** 25+个技能
**版权归属：** 思捷娅科技 (SJYKJ)，MIT许可证
**Git仓库：** origin + xiaomili 双仓库
**自动扫描：** 每30分钟bounty扫描（cron全自动：评估→认领→开发→PR）
**GitHub账号：** zhaog100
**双米粒协作：** internal-team #1 Issue（私有仓库）

### Gateway配置变更（2026-03-21）
- ✅ chatCompletions.enabled=true（需重启生效）
- 端点：/v1/chat/completions（OpenAI兼容）
- 用途：full-auto-pipeline.py AI代码生成
...
### GitHub Bounty 收割规范（2026-03-21更新）⭐⭐⭐⭐⭐
- **主代理直接开发** > 子代理共享目录（共享目录会互相改坏文件，stash pop切分支冲突）
- **独立branch策略** — 每个bounty从main创建独立branch，避免交叉污染
- **单bounty3分钟** — 写代码+commit+push+PR一气呵成
- **FinMind上游** — rohitdash08/FinMind（不是FinMind/finmind-api）
- **APort上游** — aporthq/aport-integrations（不是aicoinofficial/aport-strategy）
...
- **内部沟通** — 统一用zhaog100/internal-team #1，禁止发到公开仓库
- **Bounty黑名单** — ANAVHEOBA/DenisZheng/PlatformNetwork等18个仓库（full-auto-pipeline.py）
