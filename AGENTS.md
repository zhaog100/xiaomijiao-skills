...
**回答前必做：**
- ⚠️ 不确定的问题 → 先 `memory_search()` 查记忆，查不到再问用户
- ⚠️ 跨会话问题 → 必须查 memory/ 日志，不凭当前上下文猜测
- ⚠️ 涉及历史决策 → 必须查 MEMORY.md 确认，不凭印象

**进化机制：**
- ✅ 每次犯错后，主动更新 SOUL.md 或 MEMORY.md（记录教训）
- ✅ 每次学到新知识，更新对应文件（不是"心理笔记"）
- ✅ 发现重复犯错模式 → 写入 SOUL.md 的"禁止清单"
- ✅ 每周回顾 SOUL.md/MEMORY.md，清理过时内容
