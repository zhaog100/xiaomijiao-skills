#!/bin/bash
# 小米粒工作流程 - ClawHub协作（智能体A）
# 小米粒负责开发技能并提交米粒儿Review
...
echo "✅ 小米粒创建了分支：feature/$SKILL_NAME"

echo "小米粒正在开发 $SKILL_NAME 技能..."
read -p "开发完成？按回车继续..."

cd /root/.openclaw/workspace/skills/$SKILL_NAME
git add .
git commit -m "feat: 小米粒开发 $SKILL_NAME 技能"
git push origin feature/$SKILL_NAME
echo "✅ 小米粒已推送代码到Git"

echo "$(date) | 小米粒 | 提交PR | $SKILL_NAME | 等待米粒儿Review" > /tmp/notify_mili.txt
echo "✅ 小米粒已通知米粒儿"
...
if [ "$REVIEW_OK" = "y" ]; then
    ...
    echo "$(date) | 小米粒 | 发布 | $SKILL_NAME v$VERSION | 成功" >> /root/.openclaw/workspace/logs/clawhub_operations.log
    git add /root/.openclaw/workspace/logs/clawhub_operations.log
    git commit -m "chore: 小米粒发布 $SKILL_NAME v$VERSION"
    git push origin master
    
    echo "✅ 小米粒发布完成！🌾"
    echo "ClawHub链接：https://clawhub.com/skills/$SKILL_NAME"
else
    echo "❌ Review未通过，小米粒会修改后重新提交"
fi
